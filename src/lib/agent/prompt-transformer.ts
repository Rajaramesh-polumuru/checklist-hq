...
function generateMarkdownContext(
  ...
  commit: Commit,
  run?: Run,
  _options: { includeMetadata?: boolean; onlyIncomplete?: boolean } = {}
): string {
  const lines: string[] = [];
...
function generateJSONContext(
  ...
  commit: Commit,
  run?: Run,
  options: { includeMetadata?: boolean; onlyIncomplete?: boolean } = {}
): string {
...
function renderXMLItem(
  ...
  if (onlyIncomplete && isCompleted) return;
