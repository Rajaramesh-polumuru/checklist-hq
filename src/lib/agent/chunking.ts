...
export function chunkChecklist(
  ...
  const estimateItemTokens = (item: ChecklistItem): number => {
  ...
  const getDescendants = (itemId: string): ChecklistItem[] => {
    const children = items.filter(item => item.parent === itemId);
    const descendants: ChecklistItem[] = [...children];
  ...
  const getItemTreeTokens = (item: ChecklistItem): number => {
  ...
  let currentChunk: ChecklistItem[] = [];
  let currentTokens = 0;
  ...
  return {
    ...
    metadata: {
      ...
      itemsPerChunk: chunks.map(chunk => {
        ...
        return chunk.reduce((sum, item) => 
          sum + 1 + getDescendants(item.id).length, 0
        );
      }),
    },
  };
 ...
 */
export function estimateChecklistTokens(content: ChecklistContent): number {
  const items = Object.values(content.items);
