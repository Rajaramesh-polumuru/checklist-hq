...
export interface OrgAnalytics {
  ...
  // sparkline buckets (last 30 days, one entry per day)
  dailyRuns: DayBucket[]
...
export interface ItemAnalytics {
  ...
  avg_completion_order: number // relative position in completion sequence
}
...
export async function getOrgAnalytics(organizationId: string): Promise<OrgAnalytics> {
  ...
  const repos = (repoRows || []) as { id: string; title: string }[]
  const repoIds = repos.map(r => r.id)

  // ── 2. Runs in this org (last 30 days) ─────────────────────
  let runs: { id: string; repo_id: string; status: string; started_at: string; completed_at: string | null }[] = []
  if (repoIds.length) {
  ...
  const audits = (auditRows || []) as { action: string; created_at: string }[]
  ...
  // runs per repo
  const repoMap = new Map<string, { title: string; runs: number; completedRuns: number }>()
  ...
  for (const run of runs) {
    ...
    if (entry) {
      entry.runs++
      if (run.status === 'completed') entry.completedRuns++
    }
  }
  const runsByRepo: RepoStat[] = Array.from(repoMap.entries())
  ...
  const actionBreakdown: ActionStat[] = Array.from(actionCount.entries())
...
export async function getUserRunStats(): Promise<UserRunStats> {
...
export async function getRepoRunStats(repoId: string): Promise<RepoRunStats> {
  const { data: runs, error } = await supabase
    .from('runs')
    .select('*')
    .eq('repo_id', repoId)
...
export async function getItemAnalytics(repoId: string): Promise<ItemAnalytics[]> {
  ...
  runs?.forEach(run => {
    ...
    const content = (run as any).commits?.content?.items || {}
...
function bucket30(dates: string[]): DayBucket[] {
